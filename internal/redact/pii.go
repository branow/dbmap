...
// column-name pattern that betrays it.
//
// The match is on the NAME, never the type, because a type says nothing about
// who a value belongs to: an nvarchar(50) is a product code or a surname and
...
//
// The patterns are deliberately loose — `card` also catches `Wildcard`, `city`
// also catches `Velocity`. Withholding a harmless column costs the describer
// one column of context and is recorded as withheld; leaking a real one cannot
// be undone. The error is taken in the safe direction on purpose.
var Columns = []Column{
...
// and whether any row matched at all. The class is returned rather than a bare
// verdict so a sample can record why it withheld a column.
